# TATAINVEST -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 9
- Win rate: 11.1%
- Expectancy: -0.824R
- Profit factor: 0.01
- Max drawdown: Rs-18,948
- Avg holding days: 7.1

## Out-of-sample results
- Trades: 6
- Win rate: 33.3%
- Expectancy: -0.267R
- Profit factor: 0.73
- Max drawdown: Rs-10,954
- Avg holding days: 5.3

## Cost-viability funnel (out-of-sample)
```
    symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
TATAINVEST   96                  78                 59               19                 1                    18         8                     8         6
     TOTAL   96                  78                 59               19                 1                    18         8                     8         6
```

## Cost/risk ratio of executed trades (out-of-sample)
n=6  mean=8.2%  median=7.9%  min=6.6%  max=10.0%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: fails cost-viability or negative expectancy

