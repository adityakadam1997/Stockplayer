# IRCTC -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-10 (1241 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-09
- Out-of-sample: 2025-02-10 to 2026-08-10

## In-sample results
- Trades: 16
- Win rate: 31.2%
- Expectancy: -0.288R
- Profit factor: 0.56
- Max drawdown: Rs-21,236
- Avg holding days: 6.3

## Out-of-sample results
- Trades: 8
- Win rate: 62.5%
- Expectancy: +0.155R
- Profit factor: 1.37
- Max drawdown: Rs-6,645
- Avg holding days: 8.0

## Cost-viability funnel (out-of-sample)
```
symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
 IRCTC  103                  78                 42               36                 7                    29        11                    11         8
 TOTAL  103                  78                 42               36                 7                    29        11                    11         8
```

## Cost/risk ratio of executed trades (out-of-sample)
n=8  mean=8.9%  median=9.7%  min=5.4%  max=9.8%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: comparable to current watchlist

