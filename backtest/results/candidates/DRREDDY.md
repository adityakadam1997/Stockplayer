# DRREDDY -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 16
- Win rate: 50.0%
- Expectancy: +0.188R
- Profit factor: 1.35
- Max drawdown: Rs-8,395
- Avg holding days: 6.8

## Out-of-sample results
- Trades: 6
- Win rate: 50.0%
- Expectancy: +0.318R
- Profit factor: 1.70
- Max drawdown: Rs-3,881
- Avg holding days: 7.2

## Cost-viability funnel (out-of-sample)
```
 symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
DRREDDY  107                  82                 33               49                 3                    46        12                    12         6
  TOTAL  107                  82                 33               49                 3                    46        12                    12         6
```

## Cost/risk ratio of executed trades (out-of-sample)
n=6  mean=8.8%  median=8.9%  min=7.6%  max=9.8%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: comparable to current watchlist

