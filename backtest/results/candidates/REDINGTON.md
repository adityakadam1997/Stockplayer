# REDINGTON -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 7
- Win rate: 28.6%
- Expectancy: -0.602R
- Profit factor: 0.20
- Max drawdown: Rs-10,193
- Avg holding days: 4.9

## Out-of-sample results
- Trades: 4
- Win rate: 0.0%
- Expectancy: -0.711R
- Profit factor: 0.00
- Max drawdown: Rs-5,718
- Avg holding days: 5.8

## Cost-viability funnel (out-of-sample)
```
   symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
REDINGTON  105                  81                 50               31                 1                    30         7                     7         4
    TOTAL  105                  81                 50               31                 1                    30         7                     7         4
```

## Cost/risk ratio of executed trades (out-of-sample)
n=4  mean=7.0%  median=7.1%  min=6.0%  max=8.0%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: fails cost-viability or negative expectancy

