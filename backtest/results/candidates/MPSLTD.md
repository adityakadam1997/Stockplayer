# MPSLTD -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 14
- Win rate: 21.4%
- Expectancy: -0.470R
- Profit factor: 0.39
- Max drawdown: Rs-16,271
- Avg holding days: 6.0

## Out-of-sample results
- Trades: 4
- Win rate: 50.0%
- Expectancy: -0.339R
- Profit factor: 0.46
- Max drawdown: Rs-3,015
- Avg holding days: 7.5

## Cost-viability funnel (out-of-sample)
```
symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
MPSLTD  105                  78                 48               30                 4                    26         6                     6         4
 TOTAL  105                  78                 48               30                 4                    26         6                     6         4
```

## Cost/risk ratio of executed trades (out-of-sample)
n=4  mean=4.6%  median=4.2%  min=2.8%  max=7.1%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: fails cost-viability or negative expectancy

