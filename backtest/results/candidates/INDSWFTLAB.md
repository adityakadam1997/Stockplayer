# INDSWFTLAB -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 7
- Win rate: 42.9%
- Expectancy: +0.198R
- Profit factor: 1.36
- Max drawdown: Rs-5,909
- Avg holding days: 3.9

## Out-of-sample results
- Trades: 4
- Win rate: 25.0%
- Expectancy: -0.306R
- Profit factor: 0.59
- Max drawdown: Rs-3,339
- Avg holding days: 5.2

## Cost-viability funnel (out-of-sample)
```
    symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
INDSWFTLAB  102                  86                 62               24                 3                    21         5                     5         4
     TOTAL  102                  86                 62               24                 3                    21         5                     5         4
```

## Cost/risk ratio of executed trades (out-of-sample)
n=4  mean=4.5%  median=4.4%  min=3.4%  max=5.7%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: fails cost-viability or negative expectancy

