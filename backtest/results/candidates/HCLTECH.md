# HCLTECH -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 6
- Win rate: 50.0%
- Expectancy: +0.724R
- Profit factor: 6.29
- Max drawdown: Rs-2,110
- Avg holding days: 8.3

## Out-of-sample results
- Trades: 2
- Win rate: 50.0%
- Expectancy: +0.509R
- Profit factor: 2.73
- Max drawdown: Rs0
- Avg holding days: 8.5

## Cost-viability funnel (out-of-sample)
```
 symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
HCLTECH   94                  75                 38               37                 9                    28         3                     3         2
  TOTAL   94                  75                 38               37                 9                    28         3                     3         2
```

## Cost/risk ratio of executed trades (out-of-sample)
n=2  mean=5.9%  median=5.9%  min=4.6%  max=7.2%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: marginal -- thin sample or borderline cost/risk

