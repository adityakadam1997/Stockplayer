# ERIS -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 5
- Win rate: 20.0%
- Expectancy: -0.548R
- Profit factor: 0.42
- Max drawdown: Rs-6,560
- Avg holding days: 3.8

## Out-of-sample results
- Trades: 9
- Win rate: 44.4%
- Expectancy: -0.129R
- Profit factor: 0.77
- Max drawdown: Rs-15,148
- Avg holding days: 5.4

## Cost-viability funnel (out-of-sample)
```
symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
  ERIS  106                  86                 46               40                 4                    36        11                    11         9
 TOTAL  106                  86                 46               40                 4                    36        11                    11         9
```

## Cost/risk ratio of executed trades (out-of-sample)
n=9  mean=7.5%  median=7.1%  min=5.4%  max=9.6%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: fails cost-viability or negative expectancy

