# IIFL -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 19
- Win rate: 36.8%
- Expectancy: +0.229R
- Profit factor: 1.37
- Max drawdown: Rs-14,290
- Avg holding days: 6.3

## Out-of-sample results
- Trades: 4
- Win rate: 50.0%
- Expectancy: -0.086R
- Profit factor: 0.79
- Max drawdown: Rs-1,747
- Avg holding days: 7.5

## Cost-viability funnel (out-of-sample)
```
symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
  IIFL   97                  77                 40               37                 4                    33         6                     6         4
 TOTAL   97                  77                 40               37                 4                    33         6                     6         4
```

## Cost/risk ratio of executed trades (out-of-sample)
n=4  mean=5.0%  median=5.0%  min=4.5%  max=5.7%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: marginal -- thin sample or borderline cost/risk

