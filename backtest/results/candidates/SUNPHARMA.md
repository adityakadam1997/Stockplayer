# SUNPHARMA -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-10 (1241 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-09
- Out-of-sample: 2025-02-10 to 2026-08-10

## In-sample results
- Trades: 6
- Win rate: 16.7%
- Expectancy: -0.413R
- Profit factor: 0.35
- Max drawdown: Rs-7,728
- Avg holding days: 7.0

## Out-of-sample results
- Trades: 6
- Win rate: 50.0%
- Expectancy: -0.074R
- Profit factor: 0.90
- Max drawdown: Rs-10,313
- Avg holding days: 6.8

## Cost-viability funnel (out-of-sample)
```
   symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
SUNPHARMA  109                  90                 33               57                11                    46        10                    10         6
    TOTAL  109                  90                 33               57                11                    46        10                    10         6
```

## Cost/risk ratio of executed trades (out-of-sample)
n=6  mean=9.7%  median=9.7%  min=9.5%  max=9.9%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: marginal -- thin sample or borderline cost/risk

