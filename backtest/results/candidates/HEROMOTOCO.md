# HEROMOTOCO -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 9
- Win rate: 33.3%
- Expectancy: -0.457R
- Profit factor: 0.49
- Max drawdown: Rs-14,676
- Avg holding days: 5.7

## Out-of-sample results
- Trades: 1
- Win rate: 0.0%
- Expectancy: -1.475R
- Profit factor: 0.00
- Max drawdown: Rs0
- Avg holding days: 10.0

## Cost-viability funnel (out-of-sample)
```
    symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
HEROMOTOCO  108                  81                 43               38                 6                    32         1                     1         1
     TOTAL  108                  81                 43               38                 6                    32         1                     1         1
```

## Cost/risk ratio of executed trades (out-of-sample)
n=1  mean=8.1%  median=8.1%  min=8.1%  max=8.1%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: fails cost-viability or negative expectancy

