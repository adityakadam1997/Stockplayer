# IDEA -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-10 (1241 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-09
- Out-of-sample: 2025-02-10 to 2026-08-10

## In-sample results
- Trades: 8
- Win rate: 0.0%
- Expectancy: -0.960R
- Profit factor: 0.00
- Max drawdown: Rs-20,118
- Avg holding days: 4.6

## Out-of-sample results
- Trades: 4
- Win rate: 50.0%
- Expectancy: -0.016R
- Profit factor: 0.97
- Max drawdown: Rs-3,888
- Avg holding days: 5.5

## Cost-viability funnel (out-of-sample)
```
symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
  IDEA  118                  95                 58               37                 2                    35         5                     5         4
 TOTAL  118                  95                 58               37                 2                    35         5                     5         4
```

## Cost/risk ratio of executed trades (out-of-sample)
n=4  mean=6.1%  median=5.5%  min=3.6%  max=9.6%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: marginal -- thin sample or borderline cost/risk

