# ADANIPORTS -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 12
- Win rate: 50.0%
- Expectancy: +0.038R
- Profit factor: 1.06
- Max drawdown: Rs-9,792
- Avg holding days: 4.3

## Out-of-sample results
- Trades: 2
- Win rate: 50.0%
- Expectancy: -0.722R
- Profit factor: 0.33
- Max drawdown: Rs-6,384
- Avg holding days: 1.0

## Cost-viability funnel (out-of-sample)
```
    symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
ADANIPORTS  115                  90                 53               37                 4                    33         2                     2         2
     TOTAL  115                  90                 53               37                 4                    33         2                     2         2
```

## Cost/risk ratio of executed trades (out-of-sample)
n=2  mean=9.3%  median=9.3%  min=9.1%  max=9.5%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: fails cost-viability or negative expectancy

