# CREDITACC -- candidate vetting report

## Data coverage
2021-08-09 to 2026-08-11 (1242 trading days).

## IS/OOS split
- In-sample: 2021-08-09 to 2025-02-10
- Out-of-sample: 2025-02-11 to 2026-08-11

## In-sample results
- Trades: 12
- Win rate: 50.0%
- Expectancy: +0.208R
- Profit factor: 1.35
- Max drawdown: Rs-14,883
- Avg holding days: 4.2

## Out-of-sample results
- Trades: 2
- Win rate: 0.0%
- Expectancy: -1.215R
- Profit factor: 0.00
- Max drawdown: Rs-3,697
- Avg holding days: 5.0

## Cost-viability funnel (out-of-sample)
```
   symbol  raw  after_trend_filter  suppressed_shorts  after_long_only  invalid_geometry  after_valid_geometry  after_rr  after_cost_viability  executed
CREDITACC  113                  82                 46               36                 4                    32         3                     3         2
    TOTAL  113                  82                 46               36                 4                    32         3                     3         2
```

## Cost/risk ratio of executed trades (out-of-sample)
n=2  mean=7.9%  median=7.9%  min=6.9%  max=8.8%  (existing 15-symbol watchlist's known Cycle 3B range: ~6-18%)

## Tier: fails cost-viability or negative expectancy

